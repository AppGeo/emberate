'use strict';
/**
 *  Ember addons are often written in a way that is incompatable
...
module.exports = function copyAddonFile(file, options, cb) {
  ...
  fs.readFile(file, function(err, data) {
    ...
    if (/\.js$/.test(file)) {
      // get the addon namespace and build a regex to test for
      // require statements
      var namespace = nameSpaceFrom(file);
      var namespaceTest = new RegExp('^'+namespace, 'i');
      ...
      for (var node in AST.body) {
        ...
        if (AST.body[node].type === "ImportDeclaration" && namespaceTest.test(AST.body[node].source.value)) {
