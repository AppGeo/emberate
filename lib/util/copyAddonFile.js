...
module.exports = function copyAddonFile(file, options, cb) {
  function isAddonImport(node) {
    return options.addonList.indexOf(nameSpaceFrom(node.source.value)) !== -1;
  }
  ...
  fs.readFile(file, function(err, data) {
    ...
    if (/\.js$/.test(file)) {
      ...
      for (var node in AST.body) {
        ...
        if (isImportNode(AST.body[node]) && isAddonImport(AST.body[node])){
